...
import {
  ...
  HUB_META,
  type EarlyCareArticle,
  type EarlyCareSection,
...
function EarlyCareCard({
  section,
  tone,
  id,
}: {
  section: EarlyCareSection;
  tone: CardTone;
  id: string;
}) {
...
export default function EarlyCareArticleView({
  article,
}: {
  article: EarlyCareArticle;
}) {
  const url = `${SITE}${article.path}`;
  const hub = HUB_META[article.species];

  const breadcrumbItems = [
    { label: hub.label, href: hub.hub },
          ...
          <div className="grid lg:grid-cols-[1fr_300px] gap-10 mt-2">
            <article className="prose-content min-w-0">
              <p className="mb-3 not-prose">
                ...
                <span className="tag ml-1">{hub.label}</span>
              </p>
            ...
            >
              ...
              >
                ...
                <p className="leading-relaxed">{EARLY_CARE_SOURCE_NOTE}</p>
              </aside>
