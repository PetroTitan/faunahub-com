...
import {
  BIRD_SOURCE_NOTE,
  type BirdGuideArticle,
  type BirdGuideBreadcrumbRoot,
  type BirdGuideSection,
...
export default function BirdGuideArticleView({
  article,
  root,
  clusterLabel,
  backLabel,
}: {
  article: BirdGuideArticle;
  root: BirdGuideBreadcrumbRoot;
  clusterLabel: string;
  backLabel: string;
}) {
  ...
  const schemas = [
    breadcrumbSchema([
      { name: "Home", url: SITE },
      { name: root.label, url: `${SITE}${root.href}` },
  ...
  const sectionCount = article.sections.length + 3; // checklist + whatNotToAssume + escalation
          ...
          <div className="grid lg:grid-cols-[1fr_300px] gap-10 mt-2">
            <article className="prose-content min-w-0">
              <p className="mb-3 not-prose">
                ...
                <span className="tag ml-1">{clusterLabel}</span>
              </p>
              ...
              <div className="space-y-10 mt-10">
                {article.sections.map((section, i) => (
                  <GuideCard key={i} section={section} tone="neutral" id={`section-${i}`} />
                ))}

                <GuideCard section={article.checklist} tone="check" id="checklist" />
                <GuideCard section={article.whatNotToAssume} tone="avoid" id="what-not-to-assume" />
                <GuideCard section={article.escalation} tone="escalate" id="escalation" />
              </div>
              ...
              <div className="mt-10">
                <SourcesSection sources={article.sources} intro={BIRD_SOURCE_NOTE} />
              </div>
            </article>
            ...
            >
              ...
              >
                ...
                <p className="leading-relaxed">{BIRD_SOURCE_NOTE}</p>
              </aside>
